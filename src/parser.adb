...
package body Parser is
   ...
   procedure Load_Rocket (File_Name : String; Rocket : out Component_Access) is
      File : File_Type;
      Line : String (1 .. 1024);
      Last : Natural;
      ...
      -- We will dynamically allocate either a Nose_Cone or Body_Tube for the root
      NC : access Nose_Cone;
      BT : access Body_Tube;
      
      -- Helper to extract a float value from a tag line like <Length>0.5</Length>
      function Extract_Float (Text : String; Tag : String) return Float is
         ...
         begin
            ...
            if Parsing_Nose_Cone and then NC /= null then
               if Index (Current_Line, "<Length>") > 0 then
                  ...
                  NC.Base_Diameter := Extract_Float (Current_Line, "BaseDiameter");
               end if;
               ...
               if Index (Current_Line, "<Length>") > 0 then
                  ...
                  BT.Outer_Diameter := Extract_Float (Current_Line, "OuterDiameter");
               elsif Index (Current_Line, "<InnerDiameter>") > 0 then
                  BT.Inner_Diameter := Extract_Float (Current_Line, "InnerDiameter");
               end if;
            end if;
         end;
      end loop;
